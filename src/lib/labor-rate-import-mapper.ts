// Maps a row from a ShopVOX "Labor Rate Export List" CSV onto the shape we
// insert into the `labor_rates` table. Pure functions — used by both the
...
export type LaborRateImportRow = {
  name: string
...
export function buildRow(headerIndex: Record<string, number>, row: string[]): LaborRateImportRow {
