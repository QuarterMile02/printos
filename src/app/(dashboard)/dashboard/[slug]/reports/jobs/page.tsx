...
export default async function JobsReport(props: PageProps) {
  try {
    ...
    console.error('[reports-jobs] page crash:', err)
    return (
      <div style={{ padding: '2rem', color: '#b91c1c', fontFamily: 'monospace', whiteSpace: 'pre-wrap' }}>
        <h1 style={{ fontSize: '1.5rem', fontWeight: 'bold', marginBottom: '1rem' }}>PAGE ERROR (reports-jobs)</h1>
...
async function PageInner({ params, searchParams }: PageProps) {
  ...
  const { data: rows } = await buildBase()
    ...
    .range(from, to) as { data: JobRow[] | null; error: unknown }
