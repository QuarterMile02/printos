...
} from '@/lib/customer-import-mapper'
import { importCustomersBatch, revalidateCustomersList, type ImportBatchResult } from './actions'
  ...
  function startImport() {
    ...
    startTransition(async () => {
      for (let start = 0; start < parsed.rows.length; start += BATCH_SIZE) {
        ...
        const res = await importCustomersBatch(orgId, orgSlug, batch, csvRowOfFirst)
        if (res.error) {
      ...
      await revalidateCustomersList(orgSlug)
  ...
  return (
    <>
      <div className="mb-6 flex items-center gap-2 text-sm text-gray-500">
        <Link href={`/dashboard/${orgSlug}/customers`} className="hover:text-gray-700">Customers</Link>
        <span>/</span>
        <span className="text-gray-700">Import CSV</span>
      </div>

      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-qm-black">Import Customers</h1>
        <p className="text-sm text-qm-gray mt-1">
          Upload a CSV file of customers. Duplicates are matched by email address — or by company
          name when no email is present — and skipped. First name and last name are optional when
      ...
      {phase === 'preview' && parsed && (
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="overflow-x-auto rounded-lg border border-gray-200">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-3 py-2 text-left text-xs font-medium uppercase tracking-wide text-gray-500">Company</th>
                    <th className="px-3 py-2 text-left text-xs font-medium uppercase tracking-wide text-gray-500">First</th>
                    <th className="px-3 py-2 text-left text-xs font-medium uppercase tracking-wide text-gray-500">Last</th>
                    <th className="px-3 py-2 text-left text-xs font-medium uppercase tracking-wide text-gray-500">Email</th>
                    <th className="px-3 py-2 text-left text-xs font-medium uppercase tracking-wide text-gray-500">Phone</th>
                    <th className="px-3 py-2 text-left text-xs font-medium uppercase tracking-wide text-gray-500">City</th>
                    <th className="px-3 py-2 text-left text-xs font-medium uppercase tracking-wide text-gray-500">State</th>
                    <th className="px-3 py-2 text-left text-xs font-medium uppercase tracking-wide text-gray-500">Status</th>
                    <th className="px-3 py-2 text-left text-xs font-medium uppercase tracking-wide text-gray-500">Tags</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100 bg-white">
                  {previewRows.map((r, i) => (
                    <tr key={i}>
                      <td className="px-3 py-1.5 text-sm text-qm-black max-w-[160px] truncate" title={r.company_name ?? ''}>{r.company_name ?? <span className="text-gray-300">—</span>}</td>
                      <td className="px-3 py-1.5 text-sm text-qm-gray">{r.first_name ?? <span className="text-gray-300">—</span>}</td>
                      <td className="px-3 py-1.5 text-sm text-qm-gray">{r.last_name ?? <span className="text-gray-300">—</span>}</td>
                      <td className="px-3 py-1.5 text-sm text-qm-gray max-w-[180px] truncate" title={r.email ?? ''}>{r.email ?? <span className="text-gray-300">—</span>}</td>
                      <td className="px-3 py-1.5 text-sm text-qm-gray">{r.phone ?? <span className="text-gray-300">—</span>}</td>
                      <td className="px-3 py-1.5 text-sm text-qm-gray">{r.city ?? <span className="text-gray-300">—</span>}</td>
                      <td className="px-3 py-1.5 text-sm text-qm-gray">{r.state ?? <span className="text-gray-300">—</span>}</td>
                      <td className="px-3 py-1.5 text-sm text-qm-gray">{r.status}</td>
                      <td className="px-3 py-1.5 text-sm text-qm-gray">{r.tags.length > 0 ? r.tags.join(', ') : <span className="text-gray-300">—</span>}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="flex items-center justify-end gap-3">
            ...
            >
              Import {parsed.rows.length.toLocaleString()} customers
            </button>
          </div>
        </div>
      )}
      ...
      {phase === 'importing' && (
        <div className="rounded-xl border border-gray-200 bg-white p-8">
          <p className="text-sm font-semibold text-qm-black">Importing customers…</p>
                ...
                <table className="min-w-full text-sm">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-3 py-1.5 text-left text-xs font-medium uppercase tracking-wide text-gray-500">Customer</th>
