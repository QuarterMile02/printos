...
export default function NewQuoteForm({ orgId, orgSlug, teamMembers, currentUserId }: Props) {
  ...
  function selectCustomer(id: string, name: string, company: string | null) {
    ...
    setCustOpen(false)
    setCustResults([])
    // Reset contact and eagerly load contacts for this customer
    setContactId('')
  ...
  // ── Contact — always visible, combobox when customer selected ────────────
        ...
        {customerId && contactOpen && (
          <div className="absolute z-20 left-0 top-full mt-1 w-full rounded-md border border-gray-200 bg-white shadow-lg overflow-hidden">
            {isLoadingContacts ? (
              ...
              <p className="px-3 py-2.5 text-sm italic text-gray-400">No contacts on file for this customer.</p>
