...
export default function QuoteCustomerPicker({
  quoteId, orgId, orgSlug,
  initialCustomerId, initialCustomerName, initialCompanyName,
  initialContactId, initialContactName,
  initialContactPhone, initialContactEmail,
  ...
  function handleSave() {
    startSave(async () => {
      if (pendingCustId && pendingCustId !== customerId) {
        const res = await assignCustomerToQuote(quoteId, orgId, orgSlug, pendingCustId)
      ...
      if (pendingContactId !== undefined && pendingContactId !== contactId) {
        const res = await assignContactToQuote(quoteId, orgId, orgSlug, pendingContactId)
        if (res.error) { flash(res.error, false); return }
  ...
  const phone = initialContactPhone || customerPhone
  const email = initialContactEmail || customerEmail
    ...
    <div className="mt-1">
      ...
      <div ref={dropRef} className="relative">
        <div className="group flex items-start gap-1.5">
          <div>
            {displayCompany ? (
              <p className="font-semibold text-gray-900 leading-tight">{displayCompany}</p>
            ) : (
      ...
      <div ref={contactRef} className="relative mt-1">
        <div className="group flex items-center gap-1.5">
          <span className="text-sm text-gray-500">
            {displayContact ?? <span className="italic text-gray-400">No contact</span>}
          </span>
          {canEdit && (
            <button
              type="button"
      ...
      {(phone || email) && (
        <div className="mt-0.5 flex flex-wrap gap-x-4 gap-y-0.5 text-xs text-gray-500">
          {phone && (
            <a href={`tel:${phone}`} className="hover:text-qm-lime hover:underline">
              📞 {phone}
            </a>
          )}
          {email && (
            <a href={`mailto:${email}`} className="hover:text-qm-lime hover:underline">
              ✉ {email}
            </a>
          )}
        </div>
