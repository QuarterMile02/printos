...
import { fetchContactsForCustomer, type ContactOption } from '@/app/(dashboard)/dashboard/[slug]/assign-actions'
import { assignCustomerToInvoice, assignContactToInvoice } from './actions'
...
type Props = {
  ...
  // Gated on BOTH role (owner/admin, matches assertInvoiceEditor) and
  // is_posted=false — the picker is read-only whenever either is false.
  canReassign: boolean
}
...
export default function InvoiceCustomerPicker({
  ...
  async function selectCustomer(id: string, name: string, company: string | null) {
    ...
    setContactOpts([])
    closeDropdown()
    // Auto-load contacts and select the new customer's primary contact —
    // matches the assignCustomerToInvoice cascade (contact_id clears
    // server-side too), this just previews the likely landing state.
  ...
  function handleSave() {
    startSave(async () => {
      // Customer change cascades billing + shipping server-side —
      // saving both in one pass avoids a stale-cascade flash where the
      // old address briefly shows next to the new customer.
      if (pendingCustId && pendingCustId !== customerId) {
        const res = await assignCustomerToInvoice(invoiceId, orgId, orgSlug, pendingCustId)
        ...
        if (pendingContactId && pendingContactId !== null) {
          const cRes = await assignContactToInvoice(invoiceId, orgId, orgSlug, pendingContactId)
          if (!cRes.error) { setContactId(pendingContactId); setContactName(pendingContactName ?? null) }
        }
      } else if (pendingContactId !== undefined && pendingContactId !== contactId) {
        const res = await assignContactToInvoice(invoiceId, orgId, orgSlug, pendingContactId)
        if (res.error) { flash(res.error, false); return }
        ...
        {open && (
          <div className="absolute left-0 top-full mt-1 z-[200] w-72 rounded-lg border border-gray-200 bg-white shadow-xl overflow-hidden">
            ...
            <div className="px-3 py-2 border-t border-gray-100 flex items-center justify-end">
              <button type="button" onClick={closeDropdown} className="text-xs text-gray-500 hover:text-gray-700">
                Cancel
              </button>
            </div>
          </div>
