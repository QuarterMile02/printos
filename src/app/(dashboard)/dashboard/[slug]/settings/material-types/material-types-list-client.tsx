...
import { deleteMaterialType } from './actions-sr'
...
export default function MaterialTypesListClient({ types, usageCounts, orgSlug, orgId, userId, userRole }: Props) {
  ...
  } = useSavedView({ tableKey: 'material_types', orgId, userId, userRole })

  const activeSortRules = sortRules.length > 0 ? sortRules : DEFAULT_SORT

  const COLUMNS = useMemo((): ColumnDef<MaterialTypeRow>[] => [
    { key: 'name', label: 'Name', defaultWidth: 260, sortable: true, filterable: true, filterType: 'text', getValue: (t) => t.name },
    { key: 'materials', label: 'Materials', defaultWidth: 120, sortable: false, filterable: false },
        ...
        sorted.length === 0 ? (
          <div className="rounded-xl border border-dashed border-gray-200 bg-white py-12 text-center">
            <p className="text-sm text-gray-500">
              {search || filterRules.length > 0 ? 'No material types match the current filters.' : 'No material types yet.'}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {sorted.map((t) => (
              <MaterialTypeCard key={t.id} type={t} orgSlug={orgSlug} count={usageCounts[t.id] ?? 0} />
            ))}
          </div>
        )
      ) : (
            ...
            <thead className="bg-gray-50">
              <tr>
                {COLUMNS.map((col, i) => {
                  ...
                  const isRight = col.key === 'materials' || isActions
                  return (
                    <th
                      key={col.key}
