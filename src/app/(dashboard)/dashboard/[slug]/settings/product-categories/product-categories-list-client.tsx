...
import { deleteProductCategory } from './actions-sr'
...
const PAGE_SIZE = PRODUCT_CATEGORIES_PAGE_SIZE
...
type Props = {
  initialRows: ProductCategoryListRow[]
...
export default function ProductCategoriesListClient({
  ...
  useEffect(() => {
    ...
    Promise.all([
      client.from('product_categories').select('id', { count: 'exact', head: true }).eq('organization_id', orgId),
      client.from('product_categories').select('id', { count: 'exact', head: true }).eq('organization_id', orgId).eq('is_active', true),
      client.from('product_categories').select('id', { count: 'exact', head: true }).eq('organization_id', orgId).eq('is_active', false),
  ...
  const {
    ...
    createView,
    deleteView,
  } = useSavedView({ tableKey: 'product_categories', orgId, userId, userRole })
  ...
  const typeMap = useMemo(() => {
    ...
    for (const t of productTypes) m[t.id] = t.name
    return m
  }, [productTypes])

  // ── Column definitions ────────────────────────────────────────────────────
  const COLUMNS = useMemo((): ColumnDef<ProductCategoryListRow>[] => [
    {
    ...
    {
      key: 'product_type_id', label: 'Product Type', defaultWidth: 200,
      sortable: false, filterable: true, filterType: 'select',
      filterOptions: productTypes.map((t) => ({ label: t.name, value: t.id })),
      getValue: (c) => c.product_type_id ? typeMap[c.product_type_id] ?? null : null,
    },
    {
  ...
  // Usage counts aren't a column on `product_categories` — fetch for the
  ...
  useEffect(() => {
    ...
    createClient()
      ...
      .in('product_category_id', ids)
      .then(({ data }) => {
        if (cancelled) return
        const counts: Record<string, number> = {}
        for (const p of (data ?? []) as { product_category_id: string }[]) {
          counts[p.product_category_id] = (counts[p.product_category_id] ?? 0) + 1
        }
  ...
  if (initialRows.length === 0 && filterRules.length === 0 && !search && !loading) {
    return (
      <div className="rounded-xl border border-dashed border-gray-200 bg-white py-16 text-center">
        <p className="text-sm font-medium text-gray-900">No product categories yet</p>
        ...
        <div className={`overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm transition-opacity ${loading ? 'opacity-60 pointer-events-none' : ''}`}>
          <table className="divide-y divide-gray-200 table-fixed" style={{ width: totalWidth, minWidth: '100%' }}>
            ...
            <tbody className="divide-y divide-gray-100">
              {liveRows.map((c) => {
                const editHref = `/dashboard/${orgSlug}/settings/product-categories?edit=${c.id}`
                  ...
                  <tr key={c.id} className="group hover:bg-gray-50">
                    ...
                    <td className="whitespace-nowrap overflow-hidden">
                      <Link href={editHref} className="block px-4 py-3 text-sm text-gray-500">
                        {c.product_type_id ? typeMap[c.product_type_id] ?? '—' : '—'}
                      </Link>
                    </td>
