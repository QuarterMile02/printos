...
export default async function Page(props: PageProps) {
  try {
    ...
    console.error('[product-types] page crash:', err)
    return (
      <div style={{ padding: '2rem', color: '#b91c1c', fontFamily: 'monospace', whiteSpace: 'pre-wrap' }}>
        <h1 style={{ fontSize: '1.5rem', fontWeight: 'bold', marginBottom: '1rem' }}>PAGE ERROR (product-types)</h1>
...
async function PageInner({ params, searchParams }: PageProps) {
  ...
  const { allowed } = await checkPermission(org.id, 'settings.product_types')
  if (!allowed) {
    return (
      <div className="p-8 max-w-5xl">
        ...
        <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-6 text-sm text-amber-800">
          You don&apos;t have permission to manage product types. Contact your organization owner.
        </div>
      </div>
    )
  }
      ...
      {isPanelOpen && (
        <div className="mb-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <h2 className="text-sm font-bold uppercase tracking-wider text-gray-500 mb-4">
            {editType ? 'Edit Product Type' : 'New Product Type'}
          </h2>
          <form action={saveProductType} className="space-y-4">
            ...
            <div className="flex gap-2 pt-2">
              ...
              <Link
                href={`/dashboard/${slug}/settings/product-types`}
