...
export default function LaborRatesListClient({ rates, rateDeptMap, deptMap, orgSlug, orgId, userId, userRole }: Props) {
  ...
  } = useSavedView({ tableKey: 'labor_rates', orgId, userId, userRole })

  const activeSortRules = sortRules.length > 0 ? sortRules : DEFAULT_SORT

  function deptLabel(r: LaborRateRow): string {
  ...
  const COLUMNS = useMemo((): ColumnDef<LaborRateRow>[] => [
