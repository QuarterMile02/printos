...
import { saveLaborRate, deleteLaborRate } from './actions-sr'
  ...
  return (
    <div className="p-8 max-w-5xl">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href={`/dashboard/${slug}/settings/labor-rates?add=1`} className="rounded-md bg-qm-lime px-4 py-2 text-sm font-semibold text-white hover:brightness-110">
          + New Labor Rate
        </Link>
      </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="divide-y divide-gray-100">
            {rates.length === 0 ? (
              <tr><td colSpan={7} className="px-6 py-8 text-center text-sm text-gray-400">No labor rates yet.</td></tr>
            ) : rates.map((r) => (
              <tr key={r.id} className="hover:bg-gray-50">
                <td className="px-6 py-3">
                  <Link href={`/dashboard/${slug}/settings/labor-rates?edit=${r.id}`} className="text-sm font-medium text-gray-900 hover:text-qm-fuchsia">
                    {r.name}
                  </Link>
                </td>
                ...
                <td className="px-6 py-3 text-right">
                  <form action={deleteLaborRate} className="inline">
...
function RateForm({ rate, orgId, orgSlug }: { rate: { id: string; name: string; cost: number | null; price: number | null; formula: string | null; units: string | null; active: boolean | null } | null; orgId: string; orgSlug: string }) {
  return (
    <div className="mb-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
      <h2 className="text-sm font-bold uppercase tracking-wider text-gray-500 mb-4">{rate ? 'Edit Labor Rate' : 'New Labor Rate'}</h2>
      <form action={saveLaborRate}>
        ...
        <div className="mt-4 flex gap-3">
          ...
          <Link href={`/dashboard/${orgSlug}/settings/labor-rates`} className="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Cancel</Link>
        </div>
      </form>
    </div>
  )
