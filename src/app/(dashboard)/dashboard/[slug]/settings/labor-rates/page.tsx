        ...
        <div className="mb-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <form action={saveLaborRate}>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
              ...
              <div><label className="block text-xs font-medium text-gray-500">Formula</label><select name="formula" defaultValue={editRate?.formula ?? 'Unit'} className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-qm-lime focus:outline-none">{FORMULAS.map(f => <option key={f} value={f}>{f}</option>)}</select></div>
              <div><label className="block text-xs font-medium text-gray-500">Units</label><select name="units" defaultValue={editRate?.units ?? 'Hr'} className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-qm-lime focus:outline-none">{UNITS.map(u => <option key={u} value={u}>{u}</option>)}</select></div>
              ...
              <div><label className="block text-xs font-medium text-gray-500">Machine Charge</label><input type="number" name="machine_charge" step="0.01" defaultValue={n(editRate?.machine_charge).toFixed(2)} className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm tabular-nums focus:border-qm-lime focus:outline-none focus:ring-1 focus:ring-qm-lime" /></div>
              ...
              <div className="flex items-end gap-4 pb-1">
                ...
                <label className="flex items-center gap-1.5 text-xs cursor-pointer"><input type="checkbox" name="display_name_in_line_item" defaultChecked={editRate?.display_name_in_line_item === true} className="h-4 w-4 accent-qm-lime" />Show Name</label>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {editRate && (
                <>
                  <form action={cloneLaborRate} className="inline"><input type="hidden" name="sourceId" value={editRate.id} /><input type="hidden" name="orgId" value={org.id} /><input type="hidden" name="orgSlug" value={slug} /><input type="hidden" name="targetTable" value="labor_rates" /><button type="submit" className="rounded-md border border-gray-300 px-3 py-2 text-xs font-medium text-gray-600 hover:bg-gray-50">Clone as Labor</button></form>
                  <form action={cloneLaborRate} className="inline"><input type="hidden" name="sourceId" value={editRate.id} /><input type="hidden" name="orgId" value={org.id} /><input type="hidden" name="orgSlug" value={slug} /><input type="hidden" name="targetTable" value="machine_rates" /><button type="submit" className="rounded-md border border-gray-300 px-3 py-2 text-xs font-medium text-gray-600 hover:bg-gray-50">Clone as Machine</button></form>
                </>
              )}
            </div>
          </form>
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="divide-y divide-gray-100">
            {rates.length === 0 ? (
              <tr><td colSpan={9} className="px-4 py-8 text-center text-sm text-gray-400">No labor rates{search ? ` matching "${search}"` : ''}.</td></tr>
            ) : rates.map(r => (
