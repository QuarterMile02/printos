...
import {
  ...
  type LaborRateImportRow,
} from '@/lib/labor-rate-import-mapper'
import { importLaborRatesBatch, revalidateLaborRatesList, type ImportBatchResult } from './actions'
  ...
  function startImport() {
    ...
    startTransition(async () => {
      for (let start = 0; start < parsed.rows.length; start += BATCH_SIZE) {
        ...
        const res = await importLaborRatesBatch(orgId, orgSlug, batch, csvRowOfFirst)
        if (res.error) {
      ...
      await revalidateLaborRatesList(orgSlug)
  ...
  return (
    <>
      <div className="mb-6 flex items-center gap-2 text-sm text-gray-500">
        <Link href={`/dashboard/${orgSlug}/settings/labor-rates`} className="hover:text-gray-700">Labor Rates</Link>
        <span>/</span>
      ...
      <div className="mb-6 flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-extrabold text-qm-black">Import Labor Rates</h1>
          <p className="text-sm text-qm-gray mt-1">
            Upload a ShopVOX <code className="rounded bg-gray-100 px-1 py-0.5 text-xs">Labor Rate Export List</code> CSV.
      ...
      {phase === 'preview' && parsed && (
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-end gap-3">
            ...
            >
              Import {parsed.rows.length.toLocaleString()} labor rates
            </button>
          </div>
        </div>
      )}
      ...
      {phase === 'importing' && (
        <div className="rounded-xl border border-gray-200 bg-white p-8">
          <p className="text-sm font-semibold text-qm-black">Importing labor rates…</p>
      ...
      {phase === 'done' && summary && (
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-end gap-3">
            ...
            <button
              type="button"
              onClick={() => router.push(`/dashboard/${orgSlug}/settings/labor-rates`)}
