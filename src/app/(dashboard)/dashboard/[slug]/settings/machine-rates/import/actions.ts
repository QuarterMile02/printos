...
import type { MachineRateImportRow } from '@/lib/machine-rate-import-mapper'
...
export async function importMachineRatesBatch(
  orgId: string,
  orgSlug: string,
  ...
  if (membership.role === 'viewer') return { error: 'Viewers cannot import machine rates.' }
  ...
  const cleaned: { row: MachineRateImportRow; csvRow: number }[] = []
  rows.forEach((r, i) => {
  ...
  // 2) Build an authoritative set of every existing machine rate name in
  // this org (case-insensitive), paged in 1000-row chunks to bypass
  // PostgREST's default cap. Using a Set avoids fragile escaping.
  ...
  const toInsert: { row: MachineRateImportRow; csvRow: number }[] = []
  ...
  const { error: insertErr } = await service.from('machine_rates').insert(records)
  if (insertErr) {
    for (let i = 0; i < records.length; i++) {
      const { error } = await service.from('machine_rates').insert(records[i])
      if (error) {
        result.errors.push({
...
export async function revalidateMachineRatesList(orgSlug: string): Promise<void> {
