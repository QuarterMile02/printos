...
import {
  ...
  type MachineRateImportRow,
} from '@/lib/machine-rate-import-mapper'
import { importMachineRatesBatch, revalidateMachineRatesList, type ImportBatchResult } from './actions'
  ...
  function startImport() {
    ...
    startTransition(async () => {
      for (let start = 0; start < parsed.rows.length; start += BATCH_SIZE) {
        ...
        const res = await importMachineRatesBatch(orgId, orgSlug, batch, csvRowOfFirst)
        if (res.error) {
      ...
      await revalidateMachineRatesList(orgSlug)
  ...
  return (
    <>
      <div className="mb-6 flex items-center gap-2 text-sm text-gray-500">
        <Link href={`/dashboard/${orgSlug}/settings/machine-rates`} className="hover:text-gray-700">Machine Rates</Link>
        <span>/</span>
      ...
      <div className="mb-6 flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-extrabold text-qm-black">Import Machine Rates</h1>
          <p className="text-sm text-qm-gray mt-1">
            Upload a ShopVOX <code className="rounded bg-gray-100 px-1 py-0.5 text-xs">Machine Rate Export List</code> CSV.
      ...
      {phase === 'preview' && parsed && (
        <div className="space-y-6">
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-3 py-2 text-right text-xs font-medium uppercase tracking-wide text-gray-500">Labor Chg</th>
                ...
                <tbody className="divide-y divide-gray-100 bg-white">
                  {previewRows.map((r, i) => (
                    <tr key={i}>
                      ...
                      <td className="px-3 py-1.5 text-right text-sm tabular-nums">{r.labor_charge != null ? `$${r.labor_charge.toFixed(2)}` : '—'}</td>
                      <td className="px-3 py-1.5 text-sm text-qm-gray">{r.formula ?? '—'}</td>
          ...
          <div className="flex items-center justify-end gap-3">
            ...
            >
              Import {parsed.rows.length.toLocaleString()} machine rates
            </button>
          </div>
        </div>
      )}
      ...
      {phase === 'importing' && (
        <div className="rounded-xl border border-gray-200 bg-white p-8">
          <p className="text-sm font-semibold text-qm-black">Importing machine rates…</p>
